...
static int Funame(lua_State *L)
{
	...
	if (uname(&uts) == -1) return pusherrno(L, "uname(2) error");
	lua_createtable(L, 0, 6);
...
static int Fipaddress(lua_State *L)
{
	...
	if ((fd4 = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) return pusherrno(L, "socket(2) error");
	for (c = 0; c <= 3; c++) {
		if (!connect(fd4, (struct sockaddr *)&r4, sizeof r4)) break;
		if (c == 3) {
			inet_pton(AF_INET, "127.0.0.1", &r4.sin_addr.s_addr);
			if (connect(fd4, (struct sockaddr *)&r4, sizeof r4) == -1) return pusherrno(L, "connect(2) error");
		}
	}
	if (getsockname(fd4, (struct sockaddr *)&ip4, &ip4len) == -1) return pusherrno(L, "getsockname(2) error");
	shutdown(fd4, 2);
	inet_ntop(AF_INET, &ip4.sin_addr, ipv4, INET_ADDRSTRLEN);
	lua_pushstring(L, ipv4);
	lua_setfield(L, -2, "ipv4");

	if ((fd6 = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP)) == -1) return pusherrno(L, "socket(2) error");
	for (c = 0; c <= 3; c++) {
	...
	if (getsockname(fd6, (struct sockaddr *)&ip6, &ip6len) == -1) return pusherrno(L, "getsockname(2) error");
	shutdown(fd6, 2);
...
@return stats (TABLE)
*/
static int ifstats(lua_State *L, struct ifaddrs *i)
{
...
static int Fifaddrs(lua_State *L)
{
	...
	for (i = ifaddr; i != 0 ; i = i->ifa_next) {
		...
		if (i->ifa_addr->sa_family == AF_INET || i->ifa_addr->sa_family == AF_INET6) ipaddr(L, i);
		if (i->ifa_data && (i->ifa_addr->sa_family == AF_PACKET)) ifstats(L, i);
		lua_rawseti(L, -2, c++);
	}
	freeifaddrs(ifaddr);
	return 1;
...
@return addresses (TABLE)
*/
static int Fmacaddrs(lua_State *L)
{
	char buf[8192];
	char mac[18];
	int fd = 0;
	...
	if ((fd = socket(PF_INET, SOCK_DGRAM, 0)) == -1) return pusherrno(L, "socket(2) error");
	ifc.ifc_len = sizeof buf;
	ifc.ifc_buf = buf;
	if (ioctl(fd, SIOCGIFCONF, &ifc) == -1) return pusherrno(L, "ioctl(2) error");
	lua_newtable(L);
	for(c = 0; c < (ifc.ifc_len/sizeof (struct ifreq)); c++) {
		cifr = &ifc.ifc_req[c];
		if (ioctl(fd, SIOCGIFHWADDR, cifr) == -1) return pusherrno(L, "ioctl(2) error");
